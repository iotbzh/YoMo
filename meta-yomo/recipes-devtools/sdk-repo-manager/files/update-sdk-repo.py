...
class Package:
    ...
    def updateRevision(self, repoPackage):
...
class RepositoriesSynchronizer:
    def __init__(self, inputdir, outputdir, reponame):
    ...
    def __scanInputRPM(self):
        ...
        for archDir in listArchDir:
            ...
            listOldRpm=os.listdir( fullPathDir)

            for oldRpm in listOldRpm:
                oldPackage=Package(oldRpm, fullPathDir, repoName, archDir)
                if oldPackage.getSrcName() not in self.__dicoNewPackageSource:
                    self.__dicoNewPackageSource[oldPackage.getSrcName()]=PackageSource()
                self.__dicoNewPackageSource[oldPackage.getSrcName()].appendPkg(oldPackage)
    ...
    def __comparePackages(newPackageSource, repoPackageSource, destdir)
        for newPackage in newPackageSource.getPackages():
            ...
            if repoPackage is None:
                newPackage.copyRPM(destDir)
            else:
                if newPackage.getVersion() != repoPackage.getVersion():
                    newPackage.copyRPM(destDir)
                    repoPackage.removeRPM()
                elif newPackage.checkIfUpdateIsNeeded(repoPackage):
                    newPackage.updateRevision(repoPackage)
                    newPackage.copyRPM(destDir)
                    repoPackage.removeRPM()
...
def main():
    ...
    parser.add_argument("-a", "--output-auxilaire", help="yocto repository auxiliary destination")
    parser.add_argument("-m", "--remove-unuse", help="remove unsed rpm during update")
    ...
    sr=RepositoriesSynchronizer(args.input, args.output, args.reponame)
    sr.syncRepository()
