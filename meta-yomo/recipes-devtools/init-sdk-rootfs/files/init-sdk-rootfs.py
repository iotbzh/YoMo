...
class SdkManager():
    ...
    def __createRootfsEnv(self):
        self.__SDK_PATH = os.path.join(self._outputDir, self.__getRepoRuntimeName()+"-sdk")
        ...
        os.symlink(os.path.relpath(self.__SDKPATHNATIVE+self.__NATIVE_SYSROOT_SETUP, os.path.dirname(self.__SDKPATHNATIVE +
                                                                                                     self.__NATIVE_SYSROOT_SETUP_ALIAS)), self.__SDKPATHNATIVE+self.__NATIVE_SYSROOT_SETUP_ALIAS)
    ...
    def __createRootfsTargetRepo(self):
        archs = self.__getRuntimeConf("ALL_MULTILIB_PACKAGE_ARCHS").replace("-", "_")
        listArchs = [i for i in reversed(archs.split()) if i not in ["any", "all", "noarch"]]
        ...
        with open(self.__SDKTARGETSYSROOT+"/etc/rpmrc", "w") as f:
            f.write("arch_compat: %s: %s\n" % (self.__MACHINE_ARCH, archs if len(archs) > 0 else self.__MACHINE_ARCH))
    ...
    def __createTargetDnfTools(self):
        ...
        with open(script_path, "w") as f:
            ...
            f.write("source ${SDK_BOOTSTRAP}/environment-setup-" + self.__SDK_SYS + "\n")
...
def main():
    ...
    parser.add_argument("-v", "--verbose", help="increase output verbosity",
                        action="store_true")
    parser.add_argument("-i", "--inputJson", action='append', help="SDK JSON conf files (Can have multiple)")
